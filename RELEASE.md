...
This document describes the end-to-end release process for AllSpark iOS. The process uses **release-please** for versioning/changelog and **Fastlane** for TestFlight distribution.
...
2. **App Store Connect API Key** — required for Fastlane to upload to TestFlight without interactive login:
   ...
   - Set these environment variables (e.g. in `~/.zshrc` or a `.env` file):
     ...
     ```
   > **Note:** These env vars are account-level, not app-specific. If you already set them up for another app (e.g. Scan4D), they work here too.
...
```
feat: add new video streaming option
fix: correct reconnection logic on network drop
...
```bash
git describe --tags
# Should output: v0.4.0 (or similar)
```
